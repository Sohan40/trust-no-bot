...
export function PlayerCard({
  ...
  return (
    <article
      className={
        player.isAlive
          ? "min-w-52 rounded-md border border-[#2f374a] bg-[#131824] p-3 lg:min-w-0"
          : "min-w-52 rounded-md border border-[#34323d] bg-[#11131b] p-3 opacity-70 lg:min-w-0"
      }
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 truncate text-sm text-[#98a4b8]">
            {player.publicStyle}
          </p>
        </div>
      </div>
      ...
      {showRole && player.role ? (
        <div className="mt-3 flex items-center justify-between border-t border-[#293145] pt-3 text-xs">
          <span className="text-[#8e99ad]">Revealed role</span>
          <strong className="text-[#f3d48d]">{player.role}</strong>
        </div>
      ) : (
        <div className="mt-4">
